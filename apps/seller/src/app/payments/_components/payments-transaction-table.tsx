...
import type { PaymentStatusFilterOption, PaymentStatusFilterValue, PaymentTransactionRow } from "../types";
...
export function PaymentsTransactionTable({
  ...
  const selectedStatusLabel = statusOptions.find((option) => option.value === selectedStatus)?.label ?? "Tất cả";
    ...
    <section className="relative rounded-2xl border border-slate-200 bg-white shadow-none">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 lg:px-6 lg:py-5">
        <div className="flex flex-wrap items-center gap-3">
          <h2 className="text-2xl font-semibold tracking-tight text-slate-800">Lịch sử giao dịch &amp; Đối soát</h2>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">Trạng thái:</p>
          ...
          <div className="relative z-50 w-44">
            ...
            >
              ...
              <ChevronDown aria-hidden="true" className={cn("size-4 text-slate-500 transition-transform", isStatusOpen && "rotate-180")} />
            </Button>
                    ...
                    return (
                      <li key={option.value}>
                        ...
                        >
                          ...
                          {isSelected ? <Check aria-hidden="true" className="size-4 text-blue-600" /> : null}
                        </Button>
                      </li>
                    );
                  })}
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-slate-200 bg-slate-50/70">
            <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">NGÀY/GIỜ</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">KÊNH &amp; MÃ ĐƠN</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">LOẠI GIAO DỊCH</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">SỐ TIỀN</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">TRẠNG THÁI</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">CẢNH BÁO</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">THAO TÁC</TableHead>
          </TableRow>
        </TableHeader>
          ...
          {rows.map((row) => (
            <TableRow key={row.id} className="border-slate-200">
              <TableCell className="px-4 py-4 align-top">
                <p className="text-base font-semibold leading-tight text-slate-700">{row.dateLabel}</p>
              ...
              <TableCell className="px-4 py-4 align-top">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <p className="text-base font-semibold leading-tight text-slate-700">{row.orderCode}</p>
              ...
              <TableCell className="px-4 py-4 align-top text-sm font-semibold leading-snug text-slate-700">{row.transactionType}</TableCell>

              <TableCell className="px-4 py-4 align-top">
                <p className={cn("text-lg font-semibold leading-tight", row.amountVnd < 0 ? "text-red-500" : "text-slate-800")}>
                  {formatCurrency(row.amountVnd)}
                </p>
                {typeof row.platformFeeVnd === "number" ? (
                  <p className="mt-1.5 text-xs font-semibold text-slate-400">{formatCurrency(row.platformFeeVnd)} phí sàn</p>
                ) : null}
              </TableCell>

              <TableCell className="px-4 py-4 align-top">
                <span className={cn("inline-flex items-center gap-2 rounded-xl px-2.5 py-1.5 text-sm font-semibold", getStatusClassName(row.status))}>
              ...
              <TableCell className="px-4 py-4 align-top">
                <p className={cn("inline-flex items-center gap-2 text-sm font-semibold leading-tight", getWarningClassName(row.warningLabel))}>
                  {row.warningLabel === "—" ? null : <CircleAlert aria-hidden="true" className="size-4" />}
                  {row.warningLabel}
                </p>
              </TableCell>

              <TableCell className="px-4 py-4 align-top">
                <Button type="button" variant="outline" className={cn("h-9 rounded-xl border px-3 text-xs font-semibold tracking-widest", getActionButtonClassName(row.actionTone))}>
                  {row.actionLabel}
                </Button>
              </TableCell>
            </TableRow>
