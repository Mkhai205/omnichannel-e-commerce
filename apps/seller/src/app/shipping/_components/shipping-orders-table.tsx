    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-6 py-5">
        <div className="flex flex-wrap items-center gap-4">
          <h2 className="text-2xl font-semibold tracking-tight text-slate-900">Danh sách vận đơn</h2>
        ...
        <div className="flex items-center gap-2">
          <Button type="button" variant="outline" size="icon" className="size-10 rounded-lg border-slate-200 bg-white text-slate-500" onClick={() => {}}>
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-slate-200 bg-slate-50/80">
            <TableHead className="px-6 py-4 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">Mã vận đơn</TableHead>
            <TableHead className="px-6 py-4 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">Đơn vị vận chuyển</TableHead>
            <TableHead className="px-6 py-4 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">Trạng thái</TableHead>
            <TableHead className="px-6 py-4 text-xs font-semibold uppercase tracking-[0.13em] text-slate-400">Cập nhật cuối</TableHead>
            ...
            <TableRow key={row.id} className="border-slate-200">
              ...
              <TableCell className="px-6 py-4 align-top">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold leading-tight text-slate-700">{row.providerName}</p>
              ...
              <TableCell className="px-6 py-4 align-top">
                <span className={cn("inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wide", getStatusClassName(row.status))}>
                  {row.status}
                </span>
              </TableCell>

              <TableCell className="px-6 py-4 align-top">
                <p className="text-sm font-semibold leading-tight text-slate-700">{row.updatedAt}</p>
                <p className={cn("text-xs", row.status === "CHỜ XỬ LÝ HOÀN" ? "text-red-500" : "text-slate-400")}>{row.locationNote}</p>
              </TableCell>

              <TableCell className="px-6 py-4 align-top">
                <div className="flex items-center gap-4">
                  <button type="button" className={cn("text-xs font-semibold", row.status === "CHỜ XỬ LÝ HOÀN" ? "text-red-600" : "text-blue-500")}>{row.primaryActionLabel}</button>
                  <button type="button" className={cn("text-xs font-semibold", row.status === "CHỜ XỬ LÝ HOÀN" ? "text-red-600" : "text-slate-400")}>{row.secondaryActionLabel}</button>
