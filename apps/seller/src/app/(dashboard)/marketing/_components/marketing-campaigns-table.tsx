import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow, cn } from "@/components/ui";
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                    ...
                    {rows.map((row) => (
                        <TableRow key={row.id} className="border-slate-200">
                            ...
                            <TableCell className="px-6 py-4 align-top">
                                <span
                                    className={cn(
                                        "inline-flex rounded border px-2.5 py-1 text-xs font-semibold uppercase tracking-wide",
                                        getTypeClassName(row.type),
                                    )}
                                >
                            ...
                            <TableCell className="px-6 py-4 align-top text-sm font-semibold text-slate-500">
                                {row.dateRangeLabel}
                            </TableCell>
                            <TableCell className="px-6 py-4 align-top text-sm font-semibold text-slate-700">
                                {formatMillionVnd(row.budgetMillions)}
                            </TableCell>
                            <TableCell className="px-6 py-4 align-top text-sm font-semibold text-slate-700">
                                {formatMillionVnd(row.revenueMillions)}
                            </TableCell>
                            <TableCell className="px-6 py-4 align-top text-sm font-semibold text-blue-500">
                                {formatRoi(row.roiMultiplier)}
                            </TableCell>

                            <TableCell className="px-6 py-4 align-top">
                                <span
                                    className={cn(
                                        "inline-flex rounded-md px-3 py-1 text-xs font-semibold uppercase tracking-wide",
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 px-6 py-4 text-xs text-slate-400">
                <p className="font-semibold">
                    Hiển thị {start}-{end} trong số {totalCampaignCount.toLocaleString("en-US")}{" "}
                    chiến dịch
                </p>
