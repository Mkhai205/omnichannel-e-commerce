...
import {
  customerProfile,
  headerContent,
} from "./data/customer-service-mock-data";
import {
...
export default function CustomerServicePage() {
  const [selectedCustomerId, setSelectedCustomerId] = useState<string | null>(customerServiceLocalDatabase[0]?.customerId ?? null);
  const [chatRefreshCounter, setChatRefreshCounter] = useState(0);

  const selectedRecord = useMemo(
    () => customerServiceLocalDatabase.find((record) => record.customerId === selectedCustomerId) ?? customerServiceLocalDatabase[0],
    [selectedCustomerId],
  );

  const selectedChatMessages = useMemo(
    () => getComplaintMessagesByCustomerId(selectedRecord?.customerId ?? ""),
    [selectedRecord, chatRefreshCounter],
  );
  ...
  const handleSendMessage = (message: string) => {
    ...
    const createdMessage = appendComplaintMessageByCustomerId(selectedRecord.customerId, message, "seller");
    if (!createdMessage) {
      return;
    }
  ...
  return (
    <section className="flex w-full min-w-0 flex-col gap-6">
      ...
      <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(12rem,1fr)_minmax(0,1.85fr)_minmax(10.5rem,1.1fr)]">
        ...
        <div className="min-w-0">
          <CustomerInsightPanel profile={customerProfile} selectedRecord={selectedRecord} />
        </div>
      </div>
    </section>
  );
